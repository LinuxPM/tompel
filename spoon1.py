...
if sandi == 'aingmuda':
	print('Welcome ya yang suka gratisan :v')
else:
    ws.close()
xz = 1
status = 'tidur'
...
def on_message(ws, message):
        ...
        if evn == 'live_message' and psn[:-2] == 'geby rank':
            zz = psn[6:]
            xz = int(zz) - 1
            tanggal = datetime.today()
            nama = response3.json()['results'][xz]['author']['nickname']
            ...
            timeh = datetime.strptime(s2, formatss) - datetime.strptime(s1, formatss)
            ws.send('{"appversion":"4.3.16","event":"live_message","token":" ","useragent":"Android","message":"Info rank ' + str(xz + 1) + '  nama: ' + nama + ' judul: ' + judul + '  durasi -> ' + str(timeh) + ' "}')
        if evn == 'live_message' and psn == 'geby saya':
        ...
        if evn == 'live_message' and psn[:4] == 'geby cek':
            print('sjqjajsajajhshsajsjjsjwjwa')
            cid = psn[5:]
            headers4 = {'User-Agent': 'Mozilla/5.0'}
            response4 = requests.get(('https://id-api.spooncast.net/search/user/?keyword=' + cid + ''), headers=headers4)
        ...
        if evn == 'live_message' and psn == 'geb bangun bantu papa nyapa napa!!' and status == 'tidur':
            status = 'bangun'
            ws.send(bangun)
        if evn == 'live_message' and psn == "geby tidur gih ga ada yang nyawerr" and status == 'bangun':
        ...
        if evn == 'live_message' and psn == 'pagi':
            ws.send(fans)
        if evn == 'live_message' and psn == 'geby':
            ws.send(promot)
